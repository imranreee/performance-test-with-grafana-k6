...
export const options = {
    ...
    thresholds: {
        'http_req_duration{service:userProfile}': ['p(95)<400'],
        'http_req_failed{service:userProfile}': ['rate<0.01'],
        'checks{service:userProfile}': ['rate>0.99'],
    },
...
export function userProfileScenario() {
    ...
    const user = users[Math.floor(Math.random() * users.length)];
    const onlyUser = user.username;
    const fullUrl = `${config.baseUrl}/${onlyUser}/user-profile`;

    if (token) {
        const authHeader = { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' };
        const userProfileRes = sendPostRequest(fullUrl, { method: 'card' }, authHeader);
        check(userProfileRes, { 'User profile success': (r) => r.status === 200 });
    }
